# VoyageAI — AI Tour Planner | Complete Project, GitHub & Deployment Guide
...
This section is the GitHub-facing entry point for VoyageAI. The remainder of this document continues into the full engineering, source-code, Firebase, GCP, Docker, deployment, operations, and troubleshooting reference.

## 1. Project title and short summary

**VoyageAI — AI Tour Planner**

VoyageAI is an AI-powered conversational travel-planning application. It combines a React/Vite web interface, Firebase Authentication, a FastAPI backend, Firestore persistence, Google Gemini, LangGraph/deepagents, LangSmith observability, and real travel-data tools.

The application is designed for users who want to plan trips through natural-language conversation while keeping important user constraints such as dates, duration, budget, destination, and preferred transport explicit.

Example use case:

```text
"Plan a 4-day trip from Bangalore to Kanchipuram with trains,
hotels, non-vegetarian restaurants, sightseeing, weather,
and a shopping recommendation."
```
...
- AI-assisted trip planning through a conversational chat interface.
- Weather-aware travel planning using a dedicated Weather Agent when seasonal analysis is needed.
- Real travel tools for attractions, restaurants, routes, flights, hotels, trains, station lookup, and web research.
- Persistent chat sessions and message history stored in Firebase Firestore.
- Google Sign-In through Firebase Authentication.
- Firebase UID-based session ownership protection.
- FastAPI backend endpoints for chat, sessions, message history, and deletion.
- React/Vite frontend with Markdown rendering, session sidebar, responsive layout, loading states, and error handling.
- LangSmith tracing for model, agent, and tool observability.
- Docker-based backend packaging and Cloud Run deployment.
- Secret Manager integration so API keys are not stored in source code or Docker images.
- Cost-control configuration using Cloud Run instance limits and billing monitoring.
...
| UI | React Markdown, Lucide React, CSS |
| Authentication | Firebase Authentication / Google Sign-In |
| Frontend Firebase SDK | Firebase Web SDK |
| Backend | Python, FastAPI, Uvicorn |
| Agent orchestration | LangChain, LangGraph, deepagents |
| AI model | Google Gemini (`gemini-3.5-flash-lite`) |
| Agent observability | LangSmith |
| Database / chat storage | Firebase Firestore |
| Backend Firebase integration | Firebase Admin SDK |
| Travel data | Google Places, weather service, flights, hotels, RailRadar/trains, Geoapify, SerpAPI, Serper, web search sources used by tools |
...
> **AI provider note:** VoyageAI is the application/project name. The current LLM provider is Google Gemini; LangChain/LangGraph/deepagents provide the agent framework and orchestration.

## 4. Project structure

The complete repository layout is documented below in [Repository Structure](#4-repository-structure). The important top-level areas are:
...
The Python modules and frontend files are documented individually later in this README.
...
- a Google Cloud project for Cloud Run/Cloud Build/Artifact Registry/Secret Manager;
- the required travel API credentials;
- a Gemini API credential;
- a LangSmith API key when tracing is enabled.

The complete cloud setup is documented in [First-Time GCP Deployment — Full Sequence](#29-first-time-gcp-deployment--full-sequence).

## 6. Environment variables

### Backend

Create a local `.env` from the example configuration:
...
The current frontend requires the backend URL:
...
```env
VITE_API_URL=https://<cloud-run-service-url>
```

Firebase Web configuration values, when configured through Vite environment variables, should use the `VITE_FIREBASE_*` pattern, for example:
...
> The exact Firebase Web SDK configuration mechanism must match `frontend/ai-tour-planner-react/src/firebase.js`. Do not copy private Admin SDK credentials into frontend variables.
...
Create the frontend `.env` and set `VITE_API_URL`.

### 7.4 Return to the project root
...
The current backend exposes these application routes:

| Method | Route | Purpose |
|---|---|---|
| `POST` | `/chat` | Submit a user message and receive the AI response |
| `GET` | `/sessions` | List the authenticated user's chat sessions |
| `GET` | `/sessions/{session_id}/messages` | Load messages for one owned session |
| `DELETE` | `/sessions/{session_id}` | Delete an owned chat session and its messages |

Authentication is required through a Firebase ID token.
...
```json
{
  ...
  "message": "Plan a 3-day trip to Chennai"
}
```

See [FastAPI API endpoints](#55-backendmainpy) for implementation details.
...
The backend does not trust a `user_id` supplied by the browser. It derives the authenticated user identity from the verified Firebase token and uses that UID when reading/writing sessions.
...
For the complete production command with environment variables and Secret Manager bindings, see [Cloud Run — Canonical Production Deployment / Rebuild Command](#30-cloud-run--canonical-production-deployment--rebuild-command).

### CORS

The Firebase Hosting origin must be present in FastAPI `allow_origins` when the frontend and backend are on different origins.

### Firebase Hosting notes

Firebase Hosting deploys the Vite `dist/` output. Cloud Run hosts the FastAPI backend separately.
...
For a public GitHub repository, add screenshots under a repository folder such as:
...
Then embed them with standard Markdown:

```markdown
![VoyageAI chat interface](docs/chat.png)
```

The current repository does not claim screenshots that are not committed to Git.

## 13. Troubleshooting

Common problems and their resolutions are documented in the dedicated [Troubleshooting](#46-troubleshooting) section. The most common production issues are:

```text
CORS error
    → verify Firebase Hosting origin in backend/main.py
    → redeploy Cloud Run

Firebase authentication failure
    → verify Firebase project configuration
    → verify Google Sign-In is enabled
    → inspect browser auth/network errors

Missing environment variable
    → check local .env
    → check Cloud Run Secret Manager bindings
...
No license is assumed by this README. For a public GitHub repository, add the license you choose, for example by creating a repository-root `LICENSE` file and documenting it here.
...
For quick navigation, the GitHub-facing README structure for VoyageAI is:
...
The sections below provide the full implementation and operational detail behind this outline.
